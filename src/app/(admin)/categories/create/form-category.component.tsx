...
export default function FormCategory({
  ...
  return (
    <>
      {state?.error && (
        <div
          className='flex items-center bg-red-100 mb-4 p-4 rounded-lg text-red-800 text-sm'
          role='alert'
        >
          <svg
          ...
          >
            <path
              fill-rule='evenodd'
              d='M10 18a8 8 0 100-16 8 8 0 000 16zm1-13H9v6h2V5zm0 8H9v2h2v-2z'
              clip-rule='evenodd'
            />
          </svg>
      ...
      >
        ...
        {state?.errors?.name?.map((error, index) => (
          <li key={`error-color-${index}`}>{error}</li>
        ))}
        ...
        <input
          ...
          className={styles.input}
        />
        {state?.errors?.color?.map((error, index) => (
          <li key={`error-color-${index}`}>{error}</li>
        ))}
        <button
          type='submit'
