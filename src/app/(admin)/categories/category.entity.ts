...
export default class CategoryModel implements Category {
  ...
  static fromJson(json: any): Category {
    ...
    if (typeof json.name !== 'string')
      throw new Error("Invalid JSON: 'name' must be a string")
    if (typeof json.color !== 'string')
      throw new Error("Invalid JSON: 'color' must be a string")
    if (typeof json.id_category !== 'string')
      throw new Error("Invalid JSON: 'id_category' must be a string")
